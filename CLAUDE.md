...
### Backend (Supabase)
- **Schema**: managed via `supabase/migrations/`; current tables (see [src/integrations/supabase/types.ts](src/integrations/supabase/types.ts), which is generated — don't hand-edit): `profiles`, `user_settings`, `exercises`, `workout_plans`, `workout_plan_exercises`, `workout_sessions`, `workout_set_logs`, `body_measurements`, `body_weight_logs`, `coach_permissions`, plus a `coach_has_access(coach_email, trainee_id)` function — i.e. there's a coach/trainee permission model (see [src/pages/CoachDashboard.tsx](src/pages/CoachDashboard.tsx)). `workout_plan_exercises` also has `group_id`/`group_type` (`'superset' | 'triset'`, nullable — exercises sharing a `group_id` are grouped in the plan builder and rendered/timed together during a workout) and a nullable `notes` text column (per-exercise notes set in the plan builder). RIR/RPE fields were removed entirely (were only ever a local-only overlay via `workoutExtrasLocal.ts`, never a real column) — don't reintroduce them without an explicit ask.
  - **Coach RLS is easy to get wrong by omission**: `workout_sessions`/`workout_set_logs`/`body_weight_logs`/`workout_plans`/`workout_plan_exercises` only have `auth.uid() = user_id` policies by default — a coach viewing/editing a trainee's data needs an *explicit* additional policy gated on `coach_has_access(current_user_email(), user_id)` (see `20260813_coach_access_to_trainee_workout_data.sql`). Without it, RLS silently returns zero rows to the coach (no error) rather than failing loudly — this bit the coach dashboard's History/Progress/Weight tabs for a while. If you add a new trainee-owned table the coach dashboard needs to read, add the matching coach policy in the same migration you create the table in, don't assume it's covered by anything else.
  - **`replace_plan_exercises(p_plan_id, p_rows)`** is a Postgres RPC (`SECURITY INVOKER`, so normal RLS still applies) that atomically swaps a plan's `workout_plan_exercises` rows — `PlanEditor.tsx` calls it instead of a client-side delete-then-insert, which used to risk leaving a plan with zero exercises if the insert failed after the delete succeeded. Use this RPC (or the same pattern) for any other "replace this table's rows for X" operation rather than reintroducing a two-step client-side swap.
